...
filepath= paste0("C:/Users/madel/OneDrive - NTNU/Doktorgrad/Project 9 QPEN simulations/Quality assure Dataset 1 Salmon/")
filepath_results = paste0("C:/Users/madel/OneDrive - NTNU/Doktorgrad/Project 9 QPEN simulations/Quality assure Dataset 1 Salmon/", Sys.Date(), "_")
#load the phyloseq object
phyloseq_all_sequences = readRDS("C:/Users/madel/OneDrive - NTNU/Doktorgrad/Project 9 QPEN simulations/Quality assure Dataset 1 Salmon/2023-04-12_phyloseq_domain_bacteria.RDS")
#load the fastafile
fasta_file_experiment = "C:/Users/madel/OneDrive - NTNU/Doktorgrad/Project 5 - Amalies prosjekt - rK intake water in salmon larvea/ASV_2022/zotus.fa"
```

In this document an experiment containing outlier sequences was used to
exemplify the code and method.
...
branch_cutoff_outlier = 3#adjust if needed
...
plotname = "Dataset1_Phylogenetic_tree_all_sequences_with_outliers"
...
unique( sort(names(seq_data)) == sort(taxa_names(phyloseq_all_sequences))  ) #should only be true
...
# It can also be that the phyloseq object contains fewer ASVs/OTUs than the fasta file. In this case first filter the fastafile 

only_phyloseq_sequences <- seq_data[names(seq_data) %in% taxa_names(phyloseq_all_sequences), ]
unique( sort(names(only_phyloseq_sequences)) == sort(taxa_names(only_phyloseq_sequences))  ) #is TRUE
```
...
fasta_file_name = "Dataset1_Outlier_sequences"
...
outlier_info_df = left_join(sequence_df,taxa_df, by = "taxaID")

#save as a csv
write.csv(x = outlier_info_df, file = paste0(filepath_results, "Dataset1_outlier_phylogenetic_tree_ID_sequence.csv"))
```
...
outliers_with_accession = readRDS(paste0(filepath,"2023-06-26_outliers_BLASTed_withaccession.RDS"))
...
source(file = "C:/Users/madel/OneDrive - NTNU/Doktorgrad/Project 9 QPEN simulations/quality_control_sequences.R")
...
saveRDS(outliers_with_accession_decition, file = paste0(filepath_results, "Dataset1_Quality_control_decition.RDS"))
```

```{r load quality, echo=FALSE}
outliers_with_accession_decition = readRDS(file = paste0(filepath,"2023-06-26_Dataset1_Quality_control_decition.RDS"))
```

```{r, include=FALSE}
outliers_with_accession_decition %>% 
  group_by(Query) %>%
...
#saveRDS(quality_ps, file = paste0(filepath_results, "Dataset1_phyloseq_quality1_3151taxa.RDS") )
```
...
ggtree(phyloseq_all_sequences) + layout_circular() +
  ...
  ggplot2::ggtitle(label = paste0(Sys.Date(), " Full dataset ", ntaxa(phyloseq_all_sequences), " taxa"), 
                   subtitle = paste0(length(outliers_remove$Query %>% unique()), " outlier taxa"))
quality_tree = 
...
As can be seen from the tree, there is something strange with the two ASVs Zotu2765 and Zotu3285. These are likely also contaminants and removed from the dataset. 